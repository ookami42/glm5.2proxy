...
if ($source.Contains($marker)) {
  ...
  if ($source.Contains($versionMarker)) {
    Write-Host "Patch v3 atualizado em $($renderer.FullName)."
  } else {
    Write-Host "Patch antigo atualizado para v3 em $($renderer.FullName)."
  }
} else {
  ...
  Set-Content -LiteralPath $renderer.FullName -Value $patched -NoNewline
  Write-Host "Patch v3 inserido em $($renderer.FullName)."
}
